...
for DOID in $IDS ; do
	...
	ips=$(curl -s -X GET -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" "https://api.digitalocean.com/v2/droplets/$DOID" | jq -r '.droplet.networks.v4[] | select(.type=="private") | .ip_address' )
